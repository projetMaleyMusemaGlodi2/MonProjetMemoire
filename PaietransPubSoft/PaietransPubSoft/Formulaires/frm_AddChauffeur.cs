...
{
    public partial class frm_AddChauffeur : Form
    {
        ucChauffeur personne = null;
        int IDMalade = -1;
        object[] data;
        SplashScreenManager spl;
        public frm_AddChauffeur(SplashScreenManager spl, ucChauffeur personne, object[] data = null)
        {
            InitializeComponent();
            this.spl = spl;
        ...
        private void Save(int index)
        {
            if (index == 0)
            {
                if (this.CheckMaladeFields())
                {
                    ...
                    {
                        ALLProjetctdll.Classes.clsGlossiaireMYSQL.GetInstance().saveData("saveChauffeur", Constantes.ChauffeurDBChamps, values);
                        this.RefreshMalade();
                        personne.ChargerGrid();
                    }
        ...
        private void frm_AddChauffeur_Load(object sender, EventArgs e)
        {
            try
            {
                spl.CloseWaitForm();
