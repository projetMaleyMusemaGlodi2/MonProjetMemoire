...
class MyRaportClient extends StatefulWidget {
  final String idEngin, idClasse, menuTrie;

  const MyRaportClient({Key key, this.idEngin, this.idClasse, this.menuTrie})
      : super(key: key);
  @override
  _MyRaportClientState createState() => _MyRaportClientState();
}

class _MyRaportClientState extends State<MyRaportClient> {
  ...
  Future<List> _getMontantTot1() async {
    try {
      final response = await http.post(
          PubCon.cheminPhp + "GetRapportCourseClient.php",
          body: {
                "date1": "${f.format(date1)}",
                "date2": "${f.format(date2)}",
                "refClient": "${PubCon.userId.toString()}",
               });
      print(response.body);
